...
namespace PurpleIvy
{
    public class JobDriver_PreciseVivisection : JobDriver_DoBillPlus
    {
        ...
        protected override Toil DoBill()
        {
            ...
            var refuelableComp = tableThing.GetComp<CompRefuelable>(); //Is this Null?
            Toil toil = new Toil
            {
                initAction = delegate ()
                {
            ...
            toil.FailOn<Toil>(() =>
                this.job.bill.suspended || this.job.bill.DeletedOrDereferenced ||
                (this.job.GetTarget(TargetIndex.A).Thing is IBillGiver billGiver &&
