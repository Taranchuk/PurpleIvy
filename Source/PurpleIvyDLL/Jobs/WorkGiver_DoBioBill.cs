...
namespace PurpleIvy
{
    public class WorkGiver_DoBioBill : WorkGiver_Scanner
    {
        ...
        private static bool dummyLog(string text)
        {
        ...
        private static bool TryFindBestBillIngredients(Bill bill, Pawn pawn, Thing billGiver, List<ThingCount> chosen)
        {
            ...
            && (float)(t.Position - billGiver.Position).LengthHorizontalSquared < bill.ingredientSearchRadius
            * bill.ingredientSearchRadius && bill.IsFixedOrAllowedIngredient(t) 
            && bill.recipe.ingredients.Any((IngredientCount ingNeed) => ingNeed.filter.Allows(t)) 
            && pawn.CanReserve(t) && (bill.recipe != PurpleIvyDefOf.PI_BiomaterialsStudyRecipe ||
            PurpleIvyData.BioStudy.ContainsKey(t.def) && PurpleIvyData.BioStudy[t.def].Where
            (x => x.PrerequisitesCompleted && x.TechprintsApplied == 0 && t.Map.listerThings.ThingsOfDef
            (ThingDef.Named("Techprint_" + x.defName)).Count == 0).Count() > 0);

            //dummyLog(bill.billStack.billGiver
            //        .LabelShort + " - " + t.def.defName) &&
            //    (bill.recipe != PurpleIvyDefOf.PI_BiomaterialsStudyRecipe && dummyLog("0") ||
            //    PurpleIvyData.BioStudy.ContainsKey(t.def) && dummyLog("1") && PurpleIvyData.BioStudy[t.def]
            //    .Where(x => dummyLog(x.defName) && x.PrerequisitesCompleted && dummyLog("3") &&
            //    x.TechprintsApplied == 0 && dummyLog("4") && t.Map.listerThings.ThingsOfDef
            //    (ThingDef.Named("Techprint_" + x.defName)).Count == 0).Count() > 0 && dummyLog("5"));
